...
2. **安装 dshmarket 插件（必装）**：`dsh plugin --profile web add dshmarket`——悬浮窗的插件罗列/开关/重启依赖其 `/dsh-market/*` 接口（该路由由 dshmarket 提供，非 dsh 核心）；缺少时顶栏显示「dsh 运行中，但缺少 dshmarket 插件」
3. **启动悬浮窗**：双击桌面「DSH 插件控制台」或运行便携版
4. 悬浮窗自动连接 dsh，两列显示全部插件
